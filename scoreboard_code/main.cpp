...
void configure() {
    ...
    UCSR0B = ( 0 << RXCIE0 ) | ( 1 << RXEN0 ) | ( 0 << TXEN0 ) | ( 0 << UCSZ02 ); //disable receiver interrupt, Enable receiver, set UCSZ02 for 9-bit format

    UCSR0C = ( 1 << UCSZ01 ) | ( 1 << UCSZ00 ); // Configure 8-bit format
    UBRR0 = 103;                                  //Set baud rate at 9600
...
void awaitMessage() {
    // hunt for a valid frame: FF  cmd  u  m  l  end
    while(1){
        // find start
        if (receiveByte() == 0xFF){

        // uint8_t cmd = receiveByte();
        uint8_t u   = receiveByte();
        uint8_t m   = receiveByte();
        uint8_t l   = receiveByte();
        uint8_t end = receiveByte();
